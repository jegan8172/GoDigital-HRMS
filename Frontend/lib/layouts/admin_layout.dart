...
class _AdminLayoutState extends State<AdminLayout> {
  ...
  Future<void> _checkNewNotifications() async {
    ...
    try {
      final url = Uri.parse(
        '${ApiConfig.baseUrl}/notifications/${Uri.encodeComponent(employeeName)}',
      );
  ...
  Widget build(BuildContext context) {
    ...
    if (user?['allowed_pages'] != null) {
      if (user?['allowed_pages'] is List) {
        allowedPages = (user?['allowed_pages'] as List)
            .map((e) => e.toString())
            .toList();
    ...
    final bool showFullAdminMenu =
        isMainAdmin ||
        (userType.toLowerCase() == 'admin' && allowedPages.isEmpty);

    final filteredNavItems = allNavItems.where((item) {
      if (showFullAdminMenu) return true;
