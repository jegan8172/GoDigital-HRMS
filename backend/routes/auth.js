...
[email, email, userType || 'employee']
  ...
  return res.status(401).json({
    ...
    // Older live databases do not contain role_page_access. Keep login
    // compatible and return an empty permission list when that optional
    // table has not been introduced yet.
    let allowedPages = [];
    try {
      ...
      if (accessRows.length > 0) {
        allowedPages = JSON.parse(accessRows[0].allowed_pages || '[]');
      }
    } catch (accessError) {
